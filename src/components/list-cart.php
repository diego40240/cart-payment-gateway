<div role="cart" class="absolute -z-10 top-0 right-0 w-full max-w-md p-4 bg-white border border-gray-200 rounded-b-lg shadow sm:p-8 dark:bg-gray-800 dark:border-gray-700 -translate-y-full duration-300 ease-linear">
        ...
        <?php if (isset($_SESSION["lista_cart"]) && count($_SESSION["lista_cart"]) != 0) { ?>
            ...
            <ul role="list" data-empty-cart="false" class="divide-y divide-gray-200 dark:divide-gray-700 overflow-hidden">
                <?php foreach ($lista_cart as $key => $items) {
                    ...
                    $itemPrecio = $items[2];
                ?>
                    <li id="item-<?php echo $key ?>" class="py-3 sm:py-4 duration-300">
                        <div class="flex items-center">
                            ...
                            <div role="precio-item" class="inline-flex items-center text-base font-semibold text-gray-900 dark:text-white py-2 px-5">
                                <?php echo $itemPrecio ?>
                            </div>
                            <button class="border-x-gray-300 border-l-2 py-2 px-5" role="btn-delete-item" data-delete="<?php echo $key ?>">
            ...
            <button role="modal" data-modal-target="crud-modal" data-modal-toggle="crud-modal" class="block w-full py-2 px-5 text-sm font-medium bg-green-600 hover:opacity-95 text-white hover:underline dark:text-white dark:bg-green-500">
                Pagar
            </button>
        <?php  } else { ?>
